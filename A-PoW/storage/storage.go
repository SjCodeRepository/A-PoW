...
func NewStorage(NodeID string) *Storage {
	...
	s := &Storage{
		dbFilePath:            "./record/" + "_" + NodeID + "_database",
		blockBucket:           "block",
...
func (s *Storage) AddBlock(b *core.Block) {
	err := s.DataBase.Update(func(tx *bolt.Tx) error {
		...
		err := bbucket.Put(b.Encode(), b.Encode())
		if err != nil {
			log.Panic()
		}
		return nil
	...
	s.AddBlockHeader(b.Encode(), b.Header)
	s.UpdateNewestBlock(b.Encode())
